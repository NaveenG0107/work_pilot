...
from sqlalchemy import Boolean, Column, DateTime, ForeignKey, Integer, String, Text, Index, func, text, event
...
class UserStoryStatus(Base):
    ...
    __table_args__ = (
        Index("idx_user_story_statuses_project_id", "project_id"),
        Index("idx_user_story_statuses_deleted_at", "deleted_at"),
    )

    id = Column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid7()))
    project_id = Column(UUID(as_uuid=False), ForeignKey("projects.id", name="fk_user_story_statuses_project"), nullable=False)
