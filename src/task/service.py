...
class TaskService:
    ...
    async def upload_attachments(
        ...
        # Serialize the stored-total check with other uploads in this project.
        # A per-request check alone permits repeated batches to exceed the cap.
        await self.db.execute(
            select(Project.id).where(Project.id == project_id).with_for_update()
        )
        existing_count = int((await self.db.execute(
            select(func.count()).select_from(TaskAttachment).where(
